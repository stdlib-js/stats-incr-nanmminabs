...
var incrnanmminabs = require( './../lib' );
...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof incrnanmminabs, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function throws an error if not provided a positive integer', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			incrnanmminabs( value );
		};
	}
});

tape( 'the function returns an accumulator function', function test( t ) {
	t.strictEqual( typeof incrnanmminabs( 3 ), 'function', 'returns expected value' );
	t.end();
});

tape( 'the accumulator function computes a moving minimum absolute value incrementally', function test( t ) {
	var expected;
	var actual;
	var data;
	var acc;
	...
	acc = incrnanmminabs( 3 );

	actual = [];
	for ( i = 0; i < N; i++ ) {
		actual.push( acc( data[ i ] ) );
	}
	expected = [ 2.0, 2.0, 2.0, 2.0, 2.0, 3.0, 2.0, 2.0, 2.0, 1.0 ];
...
tape( 'if provided `NaN`, the accumulator function still returns the current minimum absolute value', function test( t ) {
	var expected;
	var data;
	var acc;
	var v;
